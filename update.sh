...
if [ -f "$APP_DIR/Apps/to-do\ list/to-do.txt" ]; then
    ...
    cp "$APP_DIR/Apps/To-Do\ List/to-do.txt" "$USERDATA_DIR/to-do.txt" || handle_error "Failed to back up user data."
else
...
# Restore user data
if [ -f "$USERDATA_DIR/to-do.txt" ]; then
    echo "Restoring user data..."
    cp "$USERDATA_DIR/to-do.txt" "$APP_DIR/Apps/To-Do\ list/to-do.txt || handle_error "Failed to restore user data."
else
