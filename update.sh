...
if [ -f "$APP_DIR/Apps/to-do\ list/to-do.txt" ]; then
    ...
    cp "$APP_DIR/Apps/to-do\ list/to-do.txt" "$USERDATA_DIR/todo.txt" || handle_error "Failed to back up user data."
else
...
if [ -f "$USERDATA_DIR/todo.txt" ]; then
    ...
    cp "$USERDATA_DIR/todo.txt" "$APP_DIR/Apps/to-do\ list/to-do.txt || handle_error "Failed to restore user data."
else
